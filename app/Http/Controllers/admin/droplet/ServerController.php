...
class ServerController extends Controller
{
    ...
    public function droplets(Request $request)
    {
        ...
        // sleep(5);
        if ($user->can('droplets.servers.view')) {
            $data = Server::join('sizes', 'servers.size', '=', 'sizes.slug')
                ->join('users', 'users.id', '=', 'servers.user_id')
                ->select([
                    'users.name as username',
                    'sizes.parvaty_price_monthly as price_monthly',
                    'sizes.parvaty_price_hourly as price_hourly',
                    'servers.*',
                ])
                ->withCount('applications');
            if ($id) {
                $data->where("servers.user_id", $id);
            }
            if ($in['filter'] !== null) {
    ...
    public function applications(Request $request)
    {
        ...
        $serverId = ($request->serverId !== null) ? $request->serverId : null;
        error_log(json_encode($sort));
        // sleep(5);
        if ($user->can('droplets.servers.view')) {
            $data = Application::select(['*']);
            if ($userId) {
                $data->where([["user_id", $userId], ['server_id', $serverId]]);
            }
            if ($in['filter'] !== null) {
                $data->where(function ($q) use ($in) {
                    foreach ($in['filter'] as $column => $value) {
    ...
    public function getResources($server)
    {
        $server = Server::find($server);
        ...
        return CommonFunctions::sendResponse(1, "Access Granted", $rsp);
    }

    public function savePrice(Request $request)
    {
        if ($request->price_monthly && $request->price_hourly && $request->id) {
            $size = ServerSize::find($request->id);
            $size->parvaty_price_monthly = $request->price_monthly;
            $size->parvaty_price_hourly = $request->price_hourly;
            $size->save();
            return CommonFunctions::sendResponse(1, "Price Saved");
        } else {
            return CommonFunctions::sendResponse(0, "All Fields Are Required");
        }
    }
