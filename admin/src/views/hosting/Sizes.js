...
// const selectedRows = [];
const Sizes = (props) => {
    const canViewServers = usePermission("droplets.sizes.view")
    ...
    const fields = [
        ...
        {
            key: 'parvaty_price_monthly',
            label: 'Parvaty $/month',
        },
        {
            key: 'parvaty_price_hourly',
            label: 'Parvaty $/hour',
        },
        {
            key: 'action',
    ...
    const getUsersData = () => {
        isLoading(true)
        new Api().get("POST", "/sizes?page=" + request.page, request, true, (data, msg) => {
            console.log(data)
            isLoading(false)
            setProjects(data)
            setSelectedRow([])
